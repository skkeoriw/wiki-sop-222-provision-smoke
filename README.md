...
```text
使用 youtube-wiki，帮我把这个 YouTube 链接跑进 skkeoriw/wiki-sop-222-provision-smoke: https://www.youtube.com/watch?v=dQw4w9WgXcQ
```
...
bash <(curl -fsSL https://skill.vyibc.com/youtube-wiki.sh) \
  --endpoint=https://youtube-wiki-222.chxyka.ccwu.cc \
  --mode=trigger \
  --repo=skkeoriw/wiki-sop-222-provision-smoke \
...
```bash
youtube-wiki trigger --repo skkeoriw/wiki-sop-222-provision-smoke --url 'https://www.youtube.com/watch?v=5MgBikgcWnY'
youtube-wiki trigger --repo skkeoriw/wiki-sop-222-provision-smoke --url 'https://www.youtube.com/watch?v=5MgBikgcWnY' --watch
youtube-wiki status --repo skkeoriw/wiki-sop-222-provision-smoke --pipeline-id '<pipeline_id>'
